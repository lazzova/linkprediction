...
public class LinkpredProblem {
	...
	public LinkpredProblem (double [][][][] graphs, byte [][] D, double alpha, double lambda, double b) {
		this.lp = new LinkPrediction(graphs, D, alpha, lambda, b);
	}

	public void optimize () {
		NonLinearConjugateGradientOptimizer opt = new NonLinearConjugateGradientOptimizer(
				NonLinearConjugateGradientOptimizer.Formula.POLAK_RIBIERE, 
				new SimplePointChecker<PointValuePair>(1.0e-6, 1.0e-4, 1000));                          // create optimizer using Polak-Ribiere formula, and convergence 
		                                                                                                // after 10^-6 difference between iterations, or 10^-3 cost function, 
		...
		MultiStartMultivariateOptimizer optimizer = 
				new MultiStartMultivariateOptimizer(opt, 800, rvg);                                      // creates multistart optimizer with 500 starting points
